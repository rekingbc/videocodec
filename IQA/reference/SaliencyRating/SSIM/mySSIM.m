function [  ] = mySSIM( runNum,dataset,datasetpath,part1,part2,part3,ImgNum,h)
B=cell(1,12);%显著方法
...
A{1,36}='Rational Model 34';
    ...
    xlswrite(strcat('SSIM_',dataset,'_',B{1,h},'.xls'),{[B{1,h},'run',int2str(runNum)]},'Sheet1',strcat(char(runNum+65),int2str(1)));
    fprintf( '%s\n',B{1,h} );
    
    file_path1  =  strcat(datasetpath,dataset,'\',B{1,h},'saliency\noise1\');%各显著算法结果路径
    sum1=0;
    for i=[1,2,19,29]
        xlswrite(strcat('SSIM_',dataset,'_',B{1,h},'.xls'),{A{1,i}},'Sheet1',strcat('A',int2str(i+2)));
        fprintf( '%s\n',A{1,i} );
        sum3=0;
        for j = 1:ImgNum
            fprintf('%d\n',j);
            image = double(imread(strcat(file_path,'g',' (',int2str(j),')','.png')));
            if i==1%I=1计算原显著算法SSIM
     ...
     if i==1
      xlswrite(strcat('SSIM_',dataset,'_',B{1,h},'.xls'),sum1/ImgNum,'Sheet1',strcat(char(runNum+65),int2str(2)));
     end
      xlswrite(strcat('SSIM_',dataset,'_',B{1,h},'.xls'),sum3/ImgNum,'Sheet1',strcat(char(runNum+65),int2str(i+2)));
    end
end
