function [  ] = myPR( runNum,dataset,datasetpath,part1,part2,part3,ImgNum,h,prsavepath)
B=cell(1,12);%显著方法
...
A{1,36}='Rational Model 34';
    ...
    xlswrite(strcat('PRAUC_',dataset,'_',B{1,h},'.xls'),{[B{1,h},'run',int2str(runNum)]},'Sheet1',strcat(char(runNum+65),int2str(1)));
    fprintf( '%s\n',B{1,h} );
    file_path1  =  strcat(datasetpath,dataset,'\',B{1,h},'saliency\noise1\');%各显著算法结果路径
    sum1=0;
    auc1 =zeros(1,ImgNum);
    precision1=zeros(ImgNum,256);
    recall1=zeros(ImgNum,256);
    for i=[1,2,19,29]
        xlswrite(strcat('PRAUC_',dataset,'_',B{1,h},'.xls'),{A{1,i}},'Sheet1',strcat('A',int2str(i+2)));
        fprintf( '%s\n',A{1,i} );
        sum3=0;
        auc3 =zeros(1,ImgNum);
        precision3=zeros(ImgNum,256);
        recall3=zeros(ImgNum,256);
        for j = 1:ImgNum
            fprintf('%d\n',j);
            ...
            if i==1%I=1计算原显著算法mse
               ...
               [precision1(j,:),recall1(j,:),fpr_temp,thresh]=prec_rec(double(image1(:)),double(image(:)));
               n=256;
               for i2=2:n
                 auc1(j) = auc1(j) + trapezoid(recall1(j,(i2-1)), recall1(j,i2), precision1(j,(i2-1)), precision1(j,i2));
               end
               auc1(j)=auc1(j)+precision1(j,1)*recall1(j,1);
               sum1=sum1+auc1(j);
            end
        end
        name = [prsavepath,B{1,h},'_',dataset,'_','run',int2str(runNum), 'ORI_precision.mat' ];
        save (name,'precision1');
        name = [prsavepath,B{1,h},'_',dataset,'_','run',int2str(runNum), 'ORI_recall.mat' ];
        save (name,'recall1');
        ...
        for j=part1
            ...
            image3=  image3*255;
            [precision3(j,:),recall3(j,:),fpr_temp,thresh]=prec_rec(double(image3(:)),double(image(:)));
             n=256;
             for i2=2:n
                 auc3(j) = auc3(j) + trapezoid(recall3(j,(i2-1)), recall3(j,i2), precision3(j,(i2-1)), precision3(j,i2));
             end
               auc3(j)=auc3(j)+precision3(j,1)*recall3(j,1);
               sum3=sum3+auc3(j);
        end
       for j=part2
           image = double(imread(strcat(file_path,'g',' (',int2str(j),')','.png')));
      ...
      for j=part3
       ...
       name = [prsavepath,B{1,h},'_',dataset,'_','run',int2str(runNum),A{1,i}, '_precision.mat' ];
       save (name,'precision3');
       name = [prsavepath,B{1,h},'_',dataset,'_','run',int2str(runNum),A{1,i},'_recall.mat' ];
       save (name,'recall3');
     if i==1
      xlswrite(strcat('PRAUC_',dataset,'_',B{1,h},'.xls'),sum1/ImgNum,'Sheet1',strcat(char(runNum+65),int2str(2)));
     end
      xlswrite(strcat('PRAUC_',dataset,'_',B{1,h},'.xls'),sum3/ImgNum,'Sheet1',strcat(char(runNum+65),int2str(i+2)));
    end
end
