function [prec, tpr, fpr, thresh] = prec_rec_AT(score, target, T)
...
%                         size as TARGET.统计符合和不符合TARGET的实例
...
%sxl comment 1 line for smap_prec_rec
%thresh = [min(score) quantile(score,qvals)];
%sxl add 1 line for smap_prec_rec
% thresh = [0:1:255];
thresh = T;
...
%sxl comment 1 line for undraw
%if (plot_pr || plot_roc)
%sxl add 1 line for undraw
if(0)
